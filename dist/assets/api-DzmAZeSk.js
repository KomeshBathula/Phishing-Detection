...
`;async function g(){return typeof chrome<`u`&&chrome.storage?new Promise(e=>{chrome.storage.local.get([`groqApiKey`],t=>{e(t.groqApiKey||``)})}):localStorage.getItem(`groqApiKey`)||``}var _={async chat(e){let t=await g();if(!t)throw Error(`Groq API Key is missing. Please add it in the PhishGuard Settings page.`);try{let n=await fetch(m,{method:`POST`,headers:{Authorization:`Bearer ${t}`,"Content-Type":`application/json`},body:JSON.stringify({model:`llama-3.3-70b-versatile`,messages:[{role:`system`,content:h},...e],temperature:.7,max_tokens:1024})});if(!n.ok){let e=await n.json();throw Error(e.error?.message||`Groq API Error: ${n.status}`)}return(await n.json()).choices[0].message}catch(e){throw console.error(`Groq Chat Error:`,e),e}},async analyzeContent(e,t){let n=await g();if(!n)throw Error(`Groq API Key is missing. Please add it in the PhishGuard Settings page.`);let r=t===`url`?`URL`:t===`email`?`email message`:`text content`,i=`
    ...
    `;try{let t=await(await fetch(m,{method:`POST`,headers:{Authorization:`Bearer ${n}`,"Content-Type":`application/json`},body:JSON.stringify({model:`llama-3.3-70b-versatile`,messages:[{role:`system`,content:i},{role:`user`,content:`Here is the ${r} to analyze, delimited by <CONTENT_START> and <CONTENT_END> tags:\n<CONTENT_START>\n${e}\n<CONTENT_END>`}],temperature:.1,response_format:{type:`json_object`}})})).json();return{...JSON.parse(t.choices[0].message.content),input:e,timestamp:new Date().toISOString()}}catch(e){throw console.error(`Groq Analysis Error:`,e),e}}},v=`http://localhost:5000/api`;async function y(e,t,n,r){try{let n=await fetch(`${v}${e}`,{...t,headers:{"Content-Type":`application/json`,...t?.headers}});if(!n.ok)throw Error(`API Error: ${n.status}`);let r=await n.json();return e.startsWith(`/analyze`)&&await p.saveScanResult(r),r}catch(i){if(i.message?.includes(`404`)||console.warn(`API call to ${e} failed, falling back to mock data.`,i),n)try{let t=await n();return e.startsWith(`/analyze`)&&await p.saveScanResult(t),t}catch(n){console.error(`AI Fallback failed, using local heuristics:`,n);let r=await l(t.body?JSON.parse(t.body).url||JSON.parse(t.body).content:``,e.split(`/`).pop());return await p.saveScanResult(r),r}return r}}var b={async analyzeURL(e){let t=e.toLowerCase();for(let n of o)if(t.includes(n)){let t={riskScore:5,classification:`Low Risk`,type:`url`,input:e,timestamp:new Date().toISOString(),reasons:[{text:`Verified legitimate domain: ${n}`,severity:`safe`}]};return await p.saveScanResult(t),t}return y(`/analyze/url`,{method:`POST`,body:JSON.stringify({url:e})},()=>_.analyzeContent(e,`url`))},async analyzeEmail(e){return y(`/analyze/email`,{method:`POST`,body:JSON.stringify({content:e})},()=>_.analyzeContent(e,`email`))},async analyzeText(e){return y(`/analyze/text`,{method:`POST`,body:JSON.stringify({content:e})},()=>_.analyzeContent(e,`text`))},async getHistory(){let e=await p.getHistory();return e&&e.length>0?e:t},async getStats(){let e=await p.getStats();return e&&e.totalScans>0?e:n},async getSimulationScenarios(){return r},async clearHistory(){await p.clearHistory()}};export{d as a,u as i,_ as n,e as o,f as r,b as t};